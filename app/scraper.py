...
def scrape_news_source(source_domain: str, topic: str) -> Optional[str]:
    """Finds the top article for a topic on a news domain and scrapes it."""
    t0 = time.time()
    urls = _duckduckgo_search(f"site:{source_domain} {topic}", max_results=1)
    if not urls:
    ...
    elapsed = int((time.time() - t0) * 1000)
    logger.info("%s scraped in %dms", source_domain, elapsed)
    return text


def scrape_all(topic: str, seed_urls: list[str] = []) -> dict[str, str]:
    """
    ...
    # 2. Default news sources
    for domain in NEWS_SOURCES:
        time.sleep(1)
        logger.info("Scraping %s", domain)
        text = scrape_news_source(domain, topic)
        if text:
            results[domain] = text
